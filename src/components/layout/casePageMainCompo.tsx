...
import { toggleFollowedCase } from "@/action/action";
...
export default function CasesPageMainCompo({ dossiersList, isPublic }: CasePageProps) {
  ...
  const dbSavedCases = dossiersList.filter((d: any) => d.isFollowed).map((d) => d.id);

  const savedCases = isAuthenticated ? (dbOptimisticSaved ?? dbSavedCases) : localSavedCases;
const isCaseSaved = selectedCase ? savedCases.includes(selectedCase.id) : false;
  ...
  const handleDownloadCase = async (e?: React.MouseEvent) => {
    e?.preventDefault();

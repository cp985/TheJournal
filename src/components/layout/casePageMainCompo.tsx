...
// import { getFollowedCase } from "@/action/action";
...
//   //   localStorage.setItem("followed_cases", JSON.stringify(newSavedCases));
//   // };

// const handleDownloadCase = async () => {
//     if (!selectedCase) return;

//     const isAuthenticated = status === "authenticated" && !!session?.data?.user;
//     const isCurrentlySaved = savedCases.includes(selectedCase.id);

//     if (isAuthenticated) {
//       try {
//         const response = await getFollowedCase(selectedCase.id);
        
//         if (response.success) {
//           setSavedCases((prev) =>
//             isCurrentlySaved
//               ? prev.filter((id) => id !== selectedCase.id)
//               : [...prev, selectedCase.id]
//           );
//         }
//       } catch (error) {
//         console.error("Errore durante l'aggiornamento su DB:", error);
//       }
//     } else {
//       const newSavedCases = isCurrentlySaved
//         ? savedCases.filter((id) => id !== selectedCase.id)
//         : [...savedCases, selectedCase.id];
...
import { CaseFileExplorer } from "@/components/layout/file-explorer-case";
import { useSession } from "next-auth/react";
import { toggleFollowedCase } from "@/action/action"; // Assicurati che l'action si chiami così
...
export default function CasesPageMainCompo({ dossiersList, isPublic }: CasePageProps) {
  ...
  // Estrazione corretta dell'oggetto sessione e del suo stato
  ...
  // Sincronizzazione al caricamento: se loggato, usa i dati dal DB
// 1. Leggi sessione e status
  const isAuthenticated = status === "authenticated" && !!session?.user;

  // 2. Stato per la gestione ANONIMA (localStorage)
  const [localSavedCases, setLocalSavedCases] = useState<string[]>(() => {
    if (typeof window === "undefined") return [];
    try {
      const stored = localStorage.getItem("followed_cases");
      return stored ? JSON.parse(stored) : [];
    } catch (e) {
      console.error("Errore nel parsing dei casi salvati:", e);
      return [];
  ...
  // 3. Stato locale temporaneo per gli aggiornamenti Optimistic (per utenti loggati)
  const [dbOptimisticSaved, setDbOptimisticSaved] = useState<string[] | null>(null);

  // 4. Calcola savedCases al volo ad ogni render senza triggering di useEffect
  const dbSavedCases = dossiersList
    .filter((d: any) => d.isFollowed)
    .map((d) => d.id);

  const savedCases = isAuthenticated
    ? (dbOptimisticSaved ?? dbSavedCases)
    : localSavedCases;
  const handleSelectCase = (item: DbDossier) => {
    router.push(`/cases?code=${encodeURIComponent(item.code)}`, {
      scroll: false,
  ...
  //   if (isAuthenticated) {
  //     // 1. UTENTE LOGGATO -> Salva/Rimuovi su Database
  //     try {
  //       const response = await toggleFollowedCase(selectedCase.id);
  ...
  //     }
  //   } else {
  //     // 2. UTENTE ANONIMO -> Salva/Rimuovi su localStorage
  //     const newSavedCases = isCurrentlySaved
  //       ? savedCases.filter((id) => id !== selectedCase.id)
  //       : [...savedCases, selectedCase.id];
  ...
  const handleDownloadCase = async () => {
    ...
    if (isAuthenticated) {
      // Aggiornamento optimistic immediato per UX reattiva
      const nextDbCases = isCurrentlySaved
        ? savedCases.filter((id) => id !== selectedCase.id)
        : [...savedCases, selectedCase.id];
      
      setDbOptimisticSaved(nextDbCases);

      try {
        const response = await toggleFollowedCase(selectedCase.id);
        if (!response?.success) {
          // Revert in caso di errore
          setDbOptimisticSaved(null);
        }
      } catch (error) {
        console.error("Errore durante l'aggiornamento su DB:", error);
        setDbOptimisticSaved(null);
      ...
      // Gestione LocalStorage per utenti anonimi
      const newSavedCases = isCurrentlySaved
        ? localSavedCases.filter((id) => id !== selectedCase.id)
        : [...localSavedCases, selectedCase.id];

      setLocalSavedCases(newSavedCases);
              ...
              >
                ...
                <div className="mt-10">
                  ...
                  <CaseFileExplorer
                    ...
                    files={selectedCase.evidences || []}
                    onDownloadCase={handleDownloadCase}
                    // OPZIONALE: se CaseFileExplorer necessita di sapere se il caso è salvato per cambiare l'UI
                    // isSaved={savedCases.includes(selectedCase.id)} 
                  />
                </div>
              </motion.div>
            ) : (

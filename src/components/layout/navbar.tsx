  ...
  if (isMapPage) {
    return (
      <>
        ...
        {/* PANNELLO DI NAVIGAZIONE A COMPARSA (con max-height e scroll verticale) */}
        <div
          className={`fixed top-0 left-0 right-0 z-50 max-h-[85vh] overflow-y-auto border-b border-zinc-800 bg-zinc-950/98 p-4 sm:p-6 backdrop-blur-lg transition-all duration-300 ease-in-out ${
            isOpen
        ...
        >
          <div className="mx-auto max-w-7xl flex flex-col gap-4">
            ...
            <nav className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 sm:gap-4 pt-2">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setIsOpen(false)}
                    className={`flex items-center justify-start gap-2 rounded-md px-3 py-2 text-xs font-medium uppercase tracking-wider transition-all ${
                      isActive
            ...
            {/* CONTROLLI SOTTO: AFFIANCATI IN ORIZZONTALE */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-t border-zinc-800/80 pt-4 mt-2">
              <div className="flex items-center self-start sm:self-auto rounded border border-zinc-800 bg-zinc-900/80 p-1 text-xs font-mono">
                <button
                  onClick={() => setLang("IT")}
              ...
              <div className="flex-1 max-w-md">
                {session ? (
                  <UserNavMobile session={session} />
                ) : (
                  <Button asChild size="sm" className="bg-amber-600 hover:bg-amber-700 text-white text-xs w-full sm:w-auto">

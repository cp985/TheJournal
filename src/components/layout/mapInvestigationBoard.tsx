...
//   (graphNodes: Node[]) => {
//     if (!rfInstance || graphNodes.length === 0) return;
...
//   };
// }, [currentLang, selectedDossierId, centerOnFirstNode]);
...
//  if (response) {
//       const graph = buildDossierGraph(currentDossier, response);
      
//       // 1. Imposta i nodi ed edges
//       setNodes(graph.nodes);
//       setEdges(graph.edges);

//       // 2. Spegni subito il caricamento in modo che React Flow monti il DOM
//       setIsLoading(false);

//       // 3. Esegui il centraggio al frame successivo
//       requestAnimationFrame(() => {
//         centerOnFirstNode(graph.nodes);
//       });
//     }
...
const handleNodeClick = (_: React.MouseEvent, node: Node) => {
    ...
    if (node.type === "folder") {
      if (node.id === "welcome-folder") return;

      const title =
        currentLang === "EN" && node.data.title_en
          ? node.data.title_en
          : node.data.title;
      const description =
        currentLang === "EN" && node.data.description_en
          ? node.data.description_en
          : node.data.description;

      let pdfUrl = "";

      if (node.data.coverUrl) {
        pdfUrl = (node.data.coverUrl as string).replace(/\/[^\/]+$/, "/dossier.pdf");
      }

      setActiveEvidence({
        type: "pdf",
        title: `${title} - Introduzione`,
        notes: description,
        fileUrl: pdfUrl,
        label: node.data.code || "FASCICOLO",
      });
      return;
    }
  ...
  const handleSelectDossier = async (dossierId: string) => {
    ...
    try {
      ...
      if (response) {
        ...
        // 2. Spegni subito il caricamento (ora toglie solo l'overlay visivo)
        setIsLoading(false);
  ...
  return (
    <div className="relative flex h-screen w-full overflow-hidden bg-zinc-950">
      ...
      <main className="relative h-full flex-1 bg-[#1c130d] bg-[radial-gradient(#2d1e15_1px,transparent_1px)] [background-size:16px_16px]">
        ...
        {/* LOADER IN SOVRAIMPRESSIONE - Ora React Flow non viene più smontato */}
        {isLoading && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-3 bg-zinc-950/80 backdrop-blur-sm font-mono text-amber-500/80 transition-all duration-300">
            <Loader2 className="h-8 w-8 animate-spin" />
            <span className="text-xs">{copy.loadingTimeline}</span>
          </div>
        )}

        {/* REACT FLOW SEMPRE ATTIVO */}
        <ReactFlow
          nodes={nodes}
          edges={edges}
          onNodesChange={onNodesChange}

      ...
      {((state.errors && Object.keys(state.errors).length > 0) ||
        ...
        >
          {state.errors && (
            <ul className="flex flex-col gap-1.5 text-xs text-red-400 font-mono">
              {Object.entries(state.errors).map(([field, messages]) => {
                ...
                const errorKey = messages[0];
                const translatedMessage =
                  (dictionary.login.errors as Record<string, string>)[errorKey] ||
                  errorKey;
          ...
          {state.message && (
            ...
            >
              {state.success
                ? (dictionary.login.success as Record<string, string>)[state.message] ||
                  state.message
                : (dictionary.login.errors as Record<string, string>)[state.message] ||
                  state.message}
            </p>
          )}
        </div>

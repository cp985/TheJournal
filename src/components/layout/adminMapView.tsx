...
export default function AdminMapView() {
  ...
  // Mock prove non associate a nessun nodo timeline (timelineId === null) per il dossier selezionato
  const mockUnassignedEvidence = [
    {
      id: "ev-101",
      title: "Foto scontrino bar",
      type: "PHOTO",
      sender: "Utente_42",
      createdAt: "10:30",
    },
    {
      id: "ev-102",
      title: "Trascrizione telefonata anonima",
      type: "DOCUMENT",
      sender: "Utente_19",
      createdAt: "Ieri",
    },
  ...
  const [selectedDossierId, setSelectedDossierId] = useState(mockDossiers[0].id);
  const activeDossier = mockDossiers.find((d) => d.id === selectedDossierId) || mockDossiers[0];
  ...
  return (
    <div className="space-y-6 font-mono text-zinc-100">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
        ...
        {/* COLONNA SINISTRA / CENTRALE (3/4): GRANDE ANTEPRIMA MAPPA */}
        <div className="lg:col-span-3 flex flex-col rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
          <div className="flex items-center justify-between border-b border-zinc-800/80 pb-3">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-400">
              <div>
                Nodi Timeline: <span className="font-bold text-zinc-100">{activeDossier.timelineCount}</span>
              </div>
              <div>
                Prove Totali: <span className="font-bold text-zinc-100">{activeDossier.evidenceCount}</span>
              </div>
        ...
        {/* COLONNA DESTRA (1/4): PROVE NON ASSOCIATE ALLA TIMELINE */}
        <div className="flex flex-col rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
          ...
          <div className="mt-3 flex-1 space-y-3 overflow-y-auto">
            {mockUnassignedEvidence.length === 0 ? (
              <p className="py-8 text-center text-xs text-zinc-600">Tutte le prove sono collegate a un fatto.</p>
            ) : (
              mockUnassignedEvidence.map((ev) => (
                <div key={ev.id} className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 text-xs transition hover:border-zinc-700">
                  <div className="flex items-center justify-between text-[10px] text-zinc-500">
                    <span className="font-mono text-amber-400/80">{ev.type}</span>
                    <span>{ev.createdAt}</span>
                  </div>
                  <div className="mt-1 font-bold text-zinc-200">{ev.title}</div>
                  <div className="mt-0.5 text-[10px] text-zinc-500">Da: {ev.sender}</div>

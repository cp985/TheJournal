...
export default function AddEvidenceDialog({
  ...
  // Stato controllato per il dossier selezionato per aggiornare la lista delle timeline
  const [selectedDossierId, setSelectedDossierId] = useState<string>(activeDossierId);

  // Recupera il dossier attivo e le relative timeline
  const currentDossier = safeDossiers.find((d) => d.code === selectedDossierId);


const rawTimelines: { id: string; title: string; title_en?: string; date: string }[] =
  (currentDossier as any)?.timeline || [];

// Ordina le timeline per data (dalla più recente alla più vecchia)
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 md:col-span-2 gap-4 items-start w-full">
              ...
              <div className="space-y-4 flex flex-col justify-between h-full">
                ...
                <div className="space-y-2">
                  ...
                  >
                    {!selectedDossierId ? (
                      <option value="">{t.placeholders?.selectDossierFirst || "Seleziona prima un Dossier..."}</option>
                    ) : currentTimelines.length === 0 ? (
                      <option value="">{t.placeholders?.noTimelines || "Nessuna timeline disponibile"}</option>
                    ) : (
                      <>
                        <option value="">{t.placeholders?.selectTimeline || "Seleziona una Timeline..."}</option>
                        {currentTimelines.map((tl) => (
                          <option key={tl.id} value={tl.id}>
                            {(lang === "EN" && tl.title_en) ? tl.title_en : tl.title} - {formatDate(tl.date)}
                          </option>
                        ))}
                      </>
                    )}
